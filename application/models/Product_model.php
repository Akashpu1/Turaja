...
public function get_product_by_sub_category($category){
  $this->db->select('*');
  $this->db->from('products');
...
public function get_count_product_by_category($category) {
  $product = array();
        ...
        $productlist[] = self::get_product_list_by_index($category);
  
    // pre($productlist);exit;
 
    return count(array_count_values(array_flatten($productlist)));
      
  
}
public function get_count_product_by_sub_category($category) {
  $product = array();
  
        $productlist[] = self::get_product_list_by_index1($category);
...
public function get_count_product_by_category_color($category) {
  ...
  if (!empty($category)) {
    ...
    // if (is_array($catlist)) {
      ...
      // }
    // }else{
      // $productlist = self::get_product_list_by_index("");
    // }
...
public function color_by_product($product) {
  ...
  $sql = "SELECT product_attributes.value AS color, COUNT(product_attributes.product_attribute_id) AS colorCount FROM attribute
          ...
          WHERE attribute.name = 'Color' AND product_attributes.product_id
...
public function get_category_id_by_name($name='') {
  ...
  return $this->db->select('id')->from('category')->get()->row();
}
...
public function get_product_list_by_index($id, $start = 0, $end = 0){
  // pre($id);exit;
  if ($end > 0) {
    return $this->db->select('id')->from('products')->where('category', $id)->limit($end, $start)->get()->result_array();
  }else{
    return $this->db->select('id')->from('products')->where('category',$id )->get()->result_array();
   }
}
public function get_product_list_by_index1($id, $start = 0, $end = 0){
  // pre($id);exit;
  if ($end > 0) {
    return $this->db->select('id')->from('products')->where('sub_category', $id)->limit($end, $start)->get()->result_array();
  }else{
    return $this->db->select('id')->from('products')->where('sub_category',$id )->get()->result_array();
   }
}
...
public function get_new_newarrivals(){
